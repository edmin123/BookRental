...
namespace BookRentalProject.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    using (var db = ApplicationDbContext.Create())
                    {
                        ...
                        var membershipType = model.MembershipTypes.Where(m => m.Id == model.MembershipTypeId).SingleOrDefault().Name.ToString();

                        if (membershipType.ToLower().Contains(SD.AdminUserRole))
                        {
                ...
                default:
                    ...
                    using (var db = ApplicationDbContext.Create())
                    {
                        return View("ExternalLoginConfirmation", new ExternalLoginConfirmationViewModel
                        {
                            ...
                            Birthday = DateTime.Now,
                            MembershipTypes = db.MembershipTypes.Where(model => !model.Name.ToLower().Contains(SD.AdminUserRole)).ToList(),
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            using (var db = ApplicationDbContext.Create())
            {
                model.MembershipTypes = db.MembershipTypes.Where(m => !m.Name.ToLower().Contains(SD.AdminUserRole)).ToList();
                return View(model);
            }
        }
